...
interface PretraitToolbarProps<TData> {
  table: Table<TData>;
  globalFilter: string;
  setGlobalFilter: (value: string) => void;
}

export function PretraitToolbar<TData>({
  table,
  globalFilter,
  setGlobalFilter,
}: PretraitToolbarProps<TData>) {
  ...
  const onDateChange = (newDateRange: DateRange | undefined) => {
    if (newDateRange?.from && newDateRange?.to) {
      table.getColumn("dateVersmentEsp")?.setFilterValue(newDateRange);
    }
    if (!newDateRange?.from && !newDateRange?.to) {
      table.getColumn("dateVersmentEsp")?.setFilterValue("");
    }
  };
